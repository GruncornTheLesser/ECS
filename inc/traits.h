...
namespace ecs {
	...
	template<typename T, id id_v> struct resource { static constexpr id ID = id_v; using type = T; };
	template<typename T, id id_v, id _> struct resource<resource<T, id_v>, _> { static constexpr id ID = id_v; using type = T; };
	...
	template<typename T>
	struct iter_tag_traits<const T> : iter_tag_traits<T> { };

	template<typename T>
	struct iter_tag_traits {
	...
	template<> struct iter_tag_traits<entity> {
		static constexpr bool primary_mixin = false;
		static constexpr bool updated_mixin = true;
	...
	template<> struct iter_tag_traits<indirect> {
		static constexpr bool primary_mixin = false;
		static constexpr bool updated_mixin = true;
		static constexpr bool ordered_mixin = true;
		static constexpr bool guarded_mixin = false;
	...
	template<auto Pred> struct iter_tag_traits<pred<Pred>> {
		static constexpr bool primary_mixin = false;
		static constexpr bool updated_mixin = true;
		static constexpr bool ordered_mixin = false;
	...
	template<typename ... Ps> struct iter_tag_traits<inc<Ps...>>   : iter_tag_traits<pred<nullptr>> { };
	template<typename ... Ps> struct iter_tag_traits<exc<Ps...>>   : iter_tag_traits<pred<nullptr>> { };
	template<typename ... Ps> struct iter_tag_traits<any<Ps...>>   : iter_tag_traits<pred<nullptr>> { };
	template<id Flag>         struct iter_tag_traits<flag<Flag>>   : iter_tag_traits<pred<nullptr>> { };
	template<auto State>      struct iter_tag_traits<state<State>> : iter_tag_traits<pred<nullptr>> { };
	...
	struct iter_traits<Iter<Ts...>> {
		...
		static constexpr std::size_t exposed_mixin_index = []->std::size_t {
			// find nth compose_mixin where iter_tag_traits<Ts>::exposed_mixin true
			if (std::size_t i = -1, j = -1; (((++i, iter_tag_traits<Ts>::exposed_mixin && ++j == I)) || ...)) return i;
			return -1;
		}();

		static constexpr std::size_t compare_mixin_index = []->std::size_t {
			// find 1st type where iter_tag_traits<Ts>::compare_mixin true and prefer entity
			if (std::size_t i = -1; ((++i, iter_tag_traits<Ts>::compare_mixin && !std::is_same_v<std::remove_const_t<Ts>, entity>) || ...)) return i;
			if (std::size_t i = -1; ((++i, iter_tag_traits<Ts>::compare_mixin) || ...)) return i;
			return -1;
		}();
		
		static constexpr std::size_t primary_mixin_index = []->std::size_t {
			// find 1st type where iter_tag_traits<Ts>::primary_mixin true and prefer from<>
			if (std::size_t i = -1; ((++i, from<Ts>::value) || ...)) return i;
			if (std::size_t i = -1; ((++i, iter_tag_traits<Ts>::primary_mixin) || ...)) return i;
			return -1;
		}();
		
		static constexpr std::size_t ordered_mixin_index = []->std::size_t {
			// find first type where iter_tag_traits<Ts>::ordered_mixin true and prefer indirect
			if (std::size_t i = -1; ((++i, std::is_same_v<std::remove_const_t<Ts>, indirect>) || ...)) return i;
			if (std::size_t i = -1; ((++i, iter_tag_traits<Ts>::ordered_mixin) || ...)) return i;
			return -1;
		}();
		
		template<std::size_t I>
		static constexpr std::size_t guarded_mixin_index = []->std::size_t {
			// find nth type where iter_tag_traits<Ts>::exposed_mixin true
			if (std::size_t i = -1, j = -1; ((++i, iter_tag_traits<Ts>::guarded_mixin && primary_mixin_index != i && ++j == I) || ...)) return i;
			return -1;
		}();
		...
		static constexpr std::size_t exposed_compose_index = []->std::size_t {
			// find nth compose_mixin where iter_tag_traits<Ts>::exposed_mixin true
			if (std::size_t i = -1, j = -1; ((iter_tag_traits<Ts>::compose_mixin && (++i, iter_tag_traits<Ts>::exposed_mixin && ++j == I)) || ...)) return i;
			return -1;
		}();
		
		static constexpr std::size_t compare_compose_index = []->std::size_t {
			// find 1st type where iter_tag_traits<Ts>::compare_mixin true and prefer entity
			if (std::size_t i = -1; ((iter_tag_traits<Ts>::compose_mixin && (++i, iter_tag_traits<Ts>::compare_mixin && !std::is_same_v<std::remove_const_t<Ts>, entity>)) || ...)) return i;
			if (std::size_t i = -1; ((iter_tag_traits<Ts>::compose_mixin && (++i, iter_tag_traits<Ts>::compare_mixin)) || ...)) return i;
			return -1;
		}();
		...
		static constexpr bool guarded = ((iter_tag_traits<Ts>::guarded_mixin && !std::is_same_v<Ts, primary_mixin>) || ...);
		...
		using update_sequence = decltype([]<std::size_t ... Is>(std::type_identity<std::tuple<std::integral_constant<std::size_t, Is>...>>) {
			...
			return std::type_identity<decltype(std::tuple_cat(
				std::conditional_t<iter_tag_traits<Ts>::updated_mixin, 
		...
		using guard_sequence = decltype([]<std::size_t ... Is>(std::index_sequence<Is...>) {
			return []<typename... Js>(std::type_identity<std::tuple<Js...>>) {
				...
				std::conditional_t<iter_tag_traits<Ts>::guarded_mixin && !std::is_same_v<primary_mixin, Ts>, 
					std::tuple<std::integral_constant<std::size_t, Is>>, 
					std::tuple<>>{}...
			))>{});
		}(std::make_index_sequence<sizeof...(Ts)>{}));
		...
		using exposed_sequence = std::index_sequence<[]<typename T>(std::type_identity<T>)->std::size_t {
			if (std::size_t i = -1; (((iter_tag_traits<Ts>::exposed_mixin && ++i), std::is_same_v<T, Ts>) || ...)) return i;
			return -1;
		...
		using compose_sequence = decltype([]<std::size_t ... Is>(std::index_sequence<Is...>) {
			return []<typename... Js>(std::type_identity<std::tuple<Js...>>) {
				...
				std::conditional_t<iter_tag_traits<Ts>::compose_mixin, 
					std::tuple<std::integral_constant<std::size_t, Is>>, 
					std::tuple<>>{}...
			))>{});
		}(std::make_index_sequence<sizeof...(Ts)>{}));
