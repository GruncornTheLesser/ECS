...
namespace ecs {
	...
	struct event_traits<T, ecs::tag::event> {
		...
		static constexpr bool enable_async = traits::event::get_attrib_strict_order_v<T>;
...
namespace ecs::event {
    template<traits::resource_class T> struct acquire {
		...
		using callback_type = std::function<void(traits::resource::get_value_t<T>&)>;
	...
	template<traits::component_class comp_T> struct init {
	private:
	...
	template<traits::component_class comp_T> struct term {
	private:
		static constexpr bool enable_storage = std::is_void_v<traits::component::get_value_t<comp_T>>;
		using value_type = util::eval_try_t<comp_T, traits::component::get_value>;
		using handle_type = ecs::traits::entity::get_handle_t<ecs::traits::component::get_entity_t<comp_T>>;
