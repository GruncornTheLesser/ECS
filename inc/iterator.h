...
namespace ecs {
	...
	template<auto S, typename base_T> requires(std::is_enum_v<decltype(S)>) 
	struct iter_mixin<state<S>, base_T> { };
	
	template<typename T, id ID, typename base_T>
	struct iter_mixin<resource<T, ID>, base_T> { };
